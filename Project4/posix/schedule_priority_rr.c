...
// fetch the task with the lowest priority and delete it from the list
Task *lowestPriorTask() {
  Task *destTask = taskList->task;
  struct node *cur = taskList;
  while(cur) { // find the task with the shortest burst
    if(cur->task->priority < destTask->priority)
      destTask = cur->task;
    cur = cur->next;
  }
...
// rearrange the list according to priority
void rearrangement() {
  while(taskList) {
    Task *newTask = lowestPriorTask();
